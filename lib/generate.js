...
import { SHARED_RBL, getSkill } from "./skills.js";
...
/**
 * Tầng gọi API chung (mọi endpoint tương thích OpenAI). Trả về text content.
 */
async function callChat(messages, { apiKey, model, baseUrl }, { jsonMode = false, maxTokens = 4000, temperature = 0.8 } = {}) {
  if (!apiKey) {
    throw new Error("Thiếu API key. Mở '⚙️ Cấu hình API', dán key Groq miễn phí (console.groq.com/keys) rồi Lưu.");
  }
  ...
  if (!resp.ok) {
    ...
    if (resp.status === 404 || (lower.includes("model") && (lower.includes("not") || lower.includes("decommission") || lower.includes("does not exist")))) {
      throw new Error(`Model '${model}' không khả dụng. Mở '⚙️ Cấu hình API' → Model, điền model khác (xem console.groq.com/docs/models — chọn model có nhãn vision nếu cần đọc ảnh).`);
    }
    if (resp.status === 429) {
...
/**
 * Skill "Kế hoạch nhanh" — trả về JSON có cấu trúc (render dạng tab).
 */
export async function generateContentPlan({ productInfo, imageDataUrl }, cfg) {
  if (!productInfo && !imageDataUrl) {
    throw new Error("Cần ít nhất thông tin sản phẩm (text) hoặc một ảnh.");
  }
  const intro = productInfo
    ? `Thông tin sản phẩm / nhu cầu:\n"""\n${productInfo}\n"""`
    : `Người dùng chỉ cung cấp ảnh dưới đây. Phân tích ảnh để suy ra sản phẩm/nhu cầu.`;
  const messages = [
    { role: "system", content: SYSTEM_PROMPT },
    { role: "user", content: buildUserContent(`${intro}\n\n${JSON_SCHEMA_INSTRUCTION}`, imageDataUrl) },
  ];
  const raw = await callChat(messages, cfg, { jsonMode: true });
  try {
...
/**
 * Chạy 1 skill RBL bất kỳ → trả về Markdown (hoặc JSON nếu skill structured).
 */
export async function generateSkill(skillId, { productInfo, imageDataUrl }, cfg) {
  const skill = getSkill(skillId);
  if (!skill) throw new Error(`Skill không tồn tại: ${skillId}`);
  if (skill.structured) return generateContentPlan({ productInfo, imageDataUrl }, cfg);
  if (!productInfo && !imageDataUrl) {
    throw new Error("Hãy nhập thông tin (text) hoặc thả một ảnh để chạy skill này.");
  }
  const intro = productInfo
    ? `Thông tin / ngữ cảnh người dùng cung cấp:\n"""\n${productInfo}\n"""`
    : `Người dùng chỉ cung cấp ảnh dưới đây — phân tích ảnh làm ngữ cảnh.`;
  const messages = [
    { role: "system", content: `${SHARED_RBL}\n\n--- SKILL: ${skill.name} ---\n${skill.prompt}` },
    { role: "user", content: buildUserContent(intro, imageDataUrl) },
  ];
  return callChat(messages, cfg, { jsonMode: false, maxTokens: 4000 });
}
